...
export default function Navbar() {
  ...
  return (
    <nav className='flex items-center justify-between sticky left-0 right-0 top-0 z-40 bg-rose-7000 lg:py-3 py-3 lg:px-[3%] px-3'>
        ...
        <div className="flex">
            <button className="flex items-center text-base px-4 py-2 bg-amber-600 rounded text-white mr-3" onClick={() => setOpenModal('login')}>
                <FiUser className='text-2xl mr-2' />
                Login
            </button>
            <button className="lg:flex hidden items-center text-base px-4 py-2 bg-amber-600 rounded text-white" onClick={() => setOpenModal('signup')}>
                <FiUser className='text-2xl mr-2' />
                Sign up
            </button>
        </div>
